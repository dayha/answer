...
export default {
  ...
  mounted() {
    ...
    if (this.sex == "女") {
      for (var i = 0; i < this.femaleDate.length; i++) {
        this.data.push(this.femaleDate[i]);
      }
      this.data.forEach((value, index, arr) => {
        if (this.data[index].subject.female) {
          for (var f = 0; f < this.data[index].subject.female.length; f++) {
            this.data[index].subject.option.splice(
              this.data[index].subject.option.length - 1,
              0,
              this.data[index].subject.female[f]
            );
          }
        }
      });
      ...
      this.data.forEach((value, index, arr) => {
        if (this.data[index].subject.male) {
          for (var m = 0; m < this.data[index].subject.male.length; m++) {
            this.data[index].subject.option.splice(
              this.data[index].subject.option.length - 1,
              0,
              this.data[index].subject.male[m]
            );
          }
        }
      });
  ...
  methods: {
    ...
    choicCompose(index, liIndex, liChildIndex) {
      //组合单选
    ...
    choice(index, liIndex, choiceStyle, liChildIndex) {
      ...
      setTimeout(() => {
        switch (
          choiceStyle //判断多选、单选、组合单选
        ) {
          case 0: //单选
            var radioTitle = this.data[index].subject.title;
            //运动情况显示是否添加数据
            if (
              radioTitle == "运动情况" &&
              (this.data[index].checkbox == "小于1小时" ||
                this.data[index].checkbox == "1-3小时" ||
                this.data[index].checkbox == "4-5小时" ||
                this.data[index].checkbox == "大于5小时")
            ) {
              if (!this.data[index].isAddData) {
                //添加
                for (var s = 0; s < this.sports.length; s++) {
            ...
            if (radioTitle == "饮酒情况" && this.data[index].checkbox == "是") {
              if (!this.data[index].isAddData) {
                //添加
                for (var s = 0; s < this.drink.length; s++) {
            ...
            break;
          case 1: //组合单选
            var childIndex = liIndex * 2 + liChildIndex;
            ...
            // 获取选中结果
            var arr = [];
            for (var i = 0; i < this.$refs.childItem.length; i++) {
...
.btn-groups {
  ...
  bottom: 0;
  left: 0;
  background-color: rgba(162, 201, 216, 0.8);
}
.btn-groups-height {
  height: 3rem;
}
...
.submit:active {
  background: rgba(68, 177, 201, 0.5);
}
.btn-groups .prev {
  position: absolute;
  left: 0;
